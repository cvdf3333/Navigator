...
export const STOCK_DISPLAY = POPULAR_STOCKS.map(s => `${s.name} (${s.symbol})`);
export const STOCK_BY_DISPLAY: Record<string, Stock> = Object.fromEntries(
  POPULAR_STOCKS.map(s => [`${s.name} (${s.symbol})`, s])
);
